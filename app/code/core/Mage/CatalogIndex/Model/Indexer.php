...
//            );
     ...
     * @return Mage_CatalogIndex_Model_Indexer
     */
    public function _walkCollectionRelation($collection, $store, $attributes = array() )
    {
        if ($store instanceof Mage_Core_Model_Website) {
        ...
        for ($i = 0; $i < $iterateCount; $i++) {
            ...
            foreach ($this->_getPriorifiedProductTypes() as $type) {
                ...
                if ($parentIds) {
                    ...
                    $this->_walkCollection($parentCollection, $storeObject, $attributes );
     ...
     * @return  Mage_CatalogIndex_Model_Indexer
     */
    protected function _walkCollection($collection, $store, $attributes = array() )
    {
        $productCount = $collection->getSize();
        if (!$productCount) {
            return $this;
